    ...
    return () => (
      <div className="p-6 bg-slate-900/50 border border-slate-800 rounded-xl text-center">
        <p className="text-xs text-slate-400">
          Archivo <span className="font-mono text-cyan-400">{path}</span> no encontrado.
        </p>
      </div>
    );
  }
...
export default function App() {
  ...
  // Control de Acceso: Muestra pantalla de Login si no ha iniciado sesión
  if (!isAuthenticated) {
    return <Login />;
  }
  ...
  // Ajusta la categoría activa dinámicamente si el rol seleccionado cambia
  useEffect(() => {
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-slate-800 pb-4 mb-4 gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          {logout && (
            <button
              onClick={logout}
              className="px-3 py-1.5 bg-red-950/40 border border-red-800/60 hover:bg-red-900/50 text-red-400 rounded-lg text-xs font-medium transition"
